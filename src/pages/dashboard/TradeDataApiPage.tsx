    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="border-b border-[#e0e3e5] px-6 py-4 flex items-center justify-between">
          <h2 className="font-bold text-[#191c1e]">Create API Key</h2>
  ...
  return (
    <div className="rounded-xl border-2 p-5 mb-6" style={{ borderColor: '#0b6c4b', background: '#a0f4ca30' }}>
      <div className="flex items-start justify-between mb-3">
        ...
        <button onClick={onDismiss} className="p-1 rounded text-[#74777f]"><span className="material-symbols-outlined" style={{ fontSize: 18 }}>close</span></button>
      </div>
          ...
          {keysLoading ? <SkeletonCard /> : (
            <div className="space-y-3">
              {allKeys.map((k) => {
                ...
                return (
                  <div key={k.id} className="bg-white rounded-xl border border-[#e0e3e5] p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <code className="text-sm font-mono text-[#74777f]">{k.keyPrefix}</code>
                      </div>
                      {k.status === 'active' && (
                    ...
                    <div>
                      <div className="flex justify-between text-xs text-[#74777f] mb-1">
      ...
      {tab === 'usage' && (
        <div className="bg-white rounded-xl border border-[#e0e3e5] overflow-hidden">
          <div className="p-4 border-b border-[#e0e3e5]">
            <p className="text-sm font-semibold text-[#191c1e]">Endpoint Usage — This Month</p>
          </div>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#f0f2f4]">
              {stats.map((s) => (
                <tr key={s.endpoint} className="hover:bg-[#f7f9fb]">
                  <td className="px-4 py-3 font-mono text-sm text-[#191c1e]">{s.endpoint}</td>
                  <td className="px-4 py-3 font-semibold text-[#191c1e]">{s.calls.toLocaleString()}</td>
      ...
      {tab === 'docs' && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-[#e0e3e5] p-6">
            ...
            <p className="text-sm text-[#74777f] mb-3">Pass your API key in the <code className="font-mono text-xs bg-[#f7f9fb] px-1 py-0.5 rounded">Authorization</code> header:</p>
            <code className="block rounded-lg px-4 py-3 text-sm font-mono text-[#191c1e]" style={{ background: '#f7f9fb', border: '1px solid #e0e3e5' }}>{'Authorization: Bearer kac_live_your_key_here'}</code>
          </div>
          <div className="bg-white rounded-xl border border-[#e0e3e5] p-6">
            <h3 className="font-bold text-[#191c1e] mb-3">Available Endpoints</h3>
            <div className="space-y-3">
              {[
...
function AdminApiView() {
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-[#191c1e]">Trade Data API — Consumer Management</h2>
        <p className="text-sm text-[#74777f] mt-0.5">Monitor institutional API consumers and their usage.</p>
      </div>
              ...
              <tbody className="divide-y divide-[#f0f2f4]">
                {allConsumers.map((c) => (
                  <tr key={c.id} className="hover:bg-[#f7f9fb] transition-colors">
                    <td className="px-4 py-3 font-semibold text-[#191c1e]">{c.institutionName}</td>
                    <td className="px-4 py-3 text-[#191c1e]">{c.apiKeyCount}</td>
                    <td className="px-4 py-3 font-medium text-[#191c1e]">{c.requestsThisMonth.toLocaleString()}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-xs text-[#74777f]">{new Date(c.createdAt).toLocaleDateString('en-NG', { timeZone: 'Africa/Lagos', day: 'numeric', month: 'short', year: 'numeric' })}</td>
                    <td className="px-4 py-3">
                      {c.status === 'active'
